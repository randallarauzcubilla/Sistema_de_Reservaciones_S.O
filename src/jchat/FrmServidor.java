...
public class FrmServidor extends javax.swing.JFrame {
    ...
    // ── NUEVO: variables de control del sorteo ──
    private javax.swing.Timer timerSorteo;
    private static final int INTERVALO_MS = 3000;
    private static final java.awt.Color COLOR_CANTADO = new java.awt.Color(52, 152, 219);
    ...
    // ── NUEVOS MÉTODOS — pegar después de los de arriba ──

    private void configurarTabla() {
        Object[][] datos = new Object[15][5];
        for (int fila = 0; fila < 15; fila++) {
            datos[fila][0] = fila + 1;
            datos[fila][1] = fila + 16;
            datos[fila][2] = fila + 31;
            datos[fila][3] = fila + 46;
            datos[fila][4] = fila + 61;
        }
        javax.swing.table.DefaultTableModel model = new javax.swing.table.DefaultTableModel(
            datos, new String[]{"B","I","N","G","O"}) {
            @Override public boolean isCellEditable(int r, int c) { return false; }
        };
        tbBingo.setModel(model);
        tbBingo.setRowHeight(24);
        ...
        javax.swing.table.DefaultTableCellRenderer renderer = new javax.swing.table.DefaultTableCellRenderer() {
            ...
            public java.awt.Component getTableCellRendererComponent(javax.swing.JTable table,
                ...
                if (value != null) {
                    int num = (int) value;
                    boolean esUltimo = !Servidor.numerosCantados.isEmpty() &&
                        Servidor.numerosCantados.stream().reduce((a,b)->b).orElse(-1) == num;
                    if (esUltimo) {
                        setBackground(COLOR_ULTIMO); setForeground(java.awt.Color.WHITE);
                    } else if (Servidor.numerosCantados.contains(num)) {
                        setBackground(COLOR_CANTADO); setForeground(java.awt.Color.WHITE);
                    } else {
                        setBackground(COLOR_NORMAL); setForeground(new java.awt.Color(52,73,94));
                    }
                }
                return this;
            }
        ...
        for (int c = 0; c < 5; c++)
            tbBingo.getColumnModel().getColumn(c).setCellRenderer(renderer);
    }

    private void iniciarSorteo() {
        if (timerSorteo != null && timerSorteo.isRunning()) return;
        Servidor.partidaActiva = true;
        btnIniciar.setEnabled(false);
        btnTerinar.setEnabled(true);
        timerSorteo = new javax.swing.Timer(INTERVALO_MS, e -> sortearNumero());
        timerSorteo.start();
        sortearNumero();
    }

    private void detenerSorteo() {
        if (timerSorteo != null) timerSorteo.stop();
        Servidor.partidaActiva = false;
        btnIniciar.setEnabled(true);
        btnTerinar.setEnabled(false);
    }

    private void sortearNumero() {
        if (Servidor.hayGanador) { detenerSorteo(); return; }
        int numero = Servidor.siguienteNumero();
        if (numero == -1) { detenerSorteo(); return; }
        String letra = letraDeNumero(numero);
        lblNumero.setText(letra + "-" + numero);
        tbBingo.repaint();
        broadcastATodos("NUMERO:" + numero);
        System.out.println("[SORTEO] " + letra + "-" + numero);
    }
